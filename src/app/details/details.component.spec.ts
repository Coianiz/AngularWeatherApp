...
export class DetailsComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(){
    ...
    this.activeRouter.paramMap.subscribe((route: any)=>{
      ...
      this.subState = this.weather.getWeatherState(this.city).subscribe((state)=> this.state = state);
      this.subTemp = this.weather.getCurrentTemp(this.city).subscribe((temperature)=> this.temp = temperature);
      this.subHum = this.weather.getCurrentHum(this.city).subscribe((humidity)=> this.hum = humidity);
      this.subWind = this.weather.getCurrentWind(this.city).subscribe((windspeed)=> this.wind = windspeed);
      this.subGeneric = this.weather.getForecast(this.city).subscribe((data: any)=>{
        console.log(data);
  ...
  ngOnDestroy(){
    this.subGeneric.unsubscribe();
    this.subHum.unsubscribe();
    this.subState.unsubscribe();
    this.subTemp.unsubscribe();
    this.subWind.unsubscribe();
  }
}